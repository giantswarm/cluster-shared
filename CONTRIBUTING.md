...
Go to `github` and create a new repo using the `ginatswarm-cluster-shared` cluster-shared. I've created
...
cd loki-app
git rm -r helm/APP-NAME-app && git commit -am "remove cluster-shared chart" && git push  # optionally remove the chart cluster-shared
